import json, frontmatter, re, requests
...
# get audio file list from url

def get_audio_list(url, filetype='.mp3'):
    audio_list = []
...
def load_psalms(settings):
    ...
    for lang_dir in base_dir.iterdir():
        if lang_dir.is_dir() and lang_dir.name in settings['languages']:
            ...
            base_url = urljoin(settings['audio_url'], f"{language}/")
            audio_list = get_audio_list(base_url)
            for md_file in lang_dir.glob('*.md'):
                ...
                for source in audio_list:
                    if psalm_data['slug'] == source.split('_')[0]:
                        key = source.split('_')[-1].split('.')[0]
